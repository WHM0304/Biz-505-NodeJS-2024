...
router.get("/", async (req, res) => {
  ...
  dbConn.query(sql, (err, result) => {
    if (err) {
      ...
      // result 값을 stList 에 담아서 list.pug 에 렌더링해라
      res.render("student/list", { stList: result });
    }
  });
});
...
router.post("/insert", (req, res) => {
  ...
  //DB 에 insert 하기 위해 배열 type 으로 변환
  // cosnt params = [req.body.st_num, req.body.st_name, req.body.st_dept];
  ...
  dbConn.query(sql, params, (err, result) => {
    if (err) {
      ...
      // INSERT(추가)가 성공한 경우 List를 보여주는 화면으로 화면전환하라
...
router.get("/:st_num/detail", (req, res) => {
  // 주소창에 있는 st_num = req.params.st_num 이다
