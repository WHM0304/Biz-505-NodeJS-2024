...
app.use(
  session({
    key: "whm0304", // 식별자, 브라우저에 저장될 cookie 이름
    secret: "whm0304@naver.com", // SessionID 암호화용 키
    cookie: {
      httpOnly: true,
...
/**
 *  모든요청 http://localhost:3000/*  로 요청한 것들(공통적으로 적용되는것들)
 * 어떤 특정 요청에 대한 routing 을 실행하는 것이 아니다
 * 모든요청에 대하여 공통으로 어떤명령을 실행하고 싶을때
 * 사용하는 router 이다
 * 이 router 코드의 끝에는 반드시 next() 함수를 실행하 주어야 한다.
 * 그렇지 않으면 다른 router 들이 전혀 실행되지 않는다.ㄴ
 */
app.use((req, res, next) => {
  res.locals = req.session;
  // next 를 안해주면 밑에있는 모든것들이 작동이 안됨
  next();
});
