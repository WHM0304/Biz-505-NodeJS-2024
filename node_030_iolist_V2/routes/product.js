 ...
 * 상품코드는 중복되면 잘대안되고, 빈(blank, empty) 값도 안된다
 * 규칙이 자릿수가 일정한 형태
 *
 * 새로운 상품코드를 생성하기 위하여
...
const makePCode = (pcode) => {
  ...
  let pCodeNum = pcode.substring(1); // 1번 index 이후의 값, 만약 00014 였다고 가정하에
  const pCodeNumLength = pCodeNum.length; // 문자열의 길이를 추출한다.
  pCodeNum = String(Number(pCodeNum) + 1); // pCode_Num 는 15가된다.
  pCodeNum = "0000000000" + pCodeNum;
  // pCodeNum.length = 10 자리 숫자 + 숫자 자릿수
...
router.post("/insert", upLoad.single("p_image"), async (req, res) => {
  ...
  if (pCode === "000") {
    // findAll() 을 실행한 결과는 비록 SELECT 된 결과가 0개 혹은 1개뿐이지만
    // 결과는 배열(리스트) type이다
