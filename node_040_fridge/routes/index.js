...
router.get("/", async (req, res, next) => {
   ...
   *   컴퓨터 표준 표시방식으로 보여준다
   * .format() 함수를 덧붙혀 원하는 형식의 날짜 문자열로 만들수 있다.
   *
   * .add(값, type)  : 현재날짜에 값을 더한 날짜 구하기
   * .subtract(값, type) : 현재 날짜에서 값을 뺀 날짜 구하기
   */
  // 오늘 부터 5일 후 날짜를 기준으로 유통기한이 임박한 상품 리스트를
  // 보고 싶다
  const exDate = moment().add(5, "days").format("YYYY-MM-DD"); // , HH:mm:ss"

  /*
  Op.gt : great then : 변수 >값
  Op.lt : less then : 변수 <값
  Ot.gte : great then and equal : 변수 >= 값
  Ot.lte : less then and equal : 변수 <= 값
